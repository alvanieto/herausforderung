...
@mock.patch('herausforderung.command.alghoritm_pipeline')
...
def test_merge_data(read_csv, adapt_data, match_data, alghoritm_pipeline, merge_data,
                    generate_file):
    read_csv.side_effect = ['data_1', 'data_2']
    adapt_data.return_value = 'adapted_data'
    alghoritm_pipeline.return_value = None
    ...
    alghoritm_pipeline.assert_called_once_with('simple')
